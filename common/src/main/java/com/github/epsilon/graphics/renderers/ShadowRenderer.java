...
public class ShadowRenderer implements IRenderer {
    ...
    public void addShadow(float x, float y, float width, float height, float rTL, float rTR, float rBR, float rBL, float blurRadius, Color color) {
        if (width <= 0.0f || height <= 0.0f || blurRadius <= 0.0f || color.getAlpha() == 0) {
            return;
        }
        ...
        float padding = blurRadius + 2.0f;
        float x2 = x + width;
        float y2 = y + height;
        int abgr = ARGB.toABGR(color.getRGB());

        addVertex(x - padding, y - padding, x, y, x2, y2, rTL, rTR, rBR, rBL, blurRadius, abgr);
        addVertex(x - padding, y2 + padding, x, y, x2, y2, rTL, rTR, rBR, rBL, blurRadius, abgr);
        addVertex(x2 + padding, y2 + padding, x, y, x2, y2, rTL, rTR, rBR, rBL, blurRadius, abgr);
        addVertex(x2 + padding, y - padding, x, y, x2, y2, rTL, rTR, rBR, rBL, blurRadius, abgr);
    }
