...
public class ShaderHolder {
    ...
    public void prepareChestOutlineTarget(RenderTarget referenceTarget) {
        ...
        CommandEncoder encoder = RenderSystem.getDevice().createCommandEncoder();
        encoder.clearColorAndDepthTextures(chestTarget.getColorTexture(), new org.joml.Vector4f(0.0f), chestTarget.getDepthTexture(), 1.0);
    }
    ...
    private RenderPipeline pipeline(String shader) {
        return RenderPipeline.builder(RenderPipelines.POST_PROCESSING_SNIPPET)
                ...
                .withBindGroupLayout(BindGroupLayout.builder().withUniform("ShaderConfig", UniformType.UNIFORM_BUFFER).build())
                .withBindGroupLayout(BindGroupLayout.builder().withSampler("InputSampler").build())
                .withCull(false)
                .build();
    }
